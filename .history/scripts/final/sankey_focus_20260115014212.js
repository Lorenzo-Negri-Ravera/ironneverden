(function() { 
    ...
    d3.csv(pathFile, d => ({
        ...
        // ==========================================
        // --- INTERAZIONE: BOTTONI ANNO (COMPACT) ---
        // ==========================================
        // Seleziona tutti i bottoni compact nel container specifico
        const yearButtons = d3.selectAll("#filter-container .btn-compact");

        yearButtons.on("click", function() {
            // 1. Gestione classe 'active' visiva
            yearButtons.classed("active", false); // Rimuovi da tutti
            d3.select(this).classed("active", true); // Aggiungi a quello cliccato

            // 2. Recupera l'anno dal data-attribute HTML
            const selectedYear = d3.select(this).attr("data-type");
            currentYear = +selectedYear;
            
            console.log("Anno cambiato:", currentYear);
            updateChart();
        });

        // ==========================================
        // --- INTERAZIONE: SLIDER MESE ---
        // ==========================================
    ...
    function updateChart() {
        ...
        // Ordina e prendi Top 15
        flows.sort((a, b) => b.value - a.value);
        flows = flows.slice(0, 15);

        // Prepara Nodi e Link per D3 Sankey
