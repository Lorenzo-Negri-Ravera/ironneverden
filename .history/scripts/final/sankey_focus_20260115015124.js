(function() { 
    ...
    d3.csv(pathFile, d => ({
        ...
        globalData = rawData;
        console.log(`Sankey - Dati caricati: ${globalData.length} righe.`);
        ...
        const specificColors = {
            "Russia":    "#003f5c", "Ukraine":   "#374c80", "Poland":    "#7a5195",
            "Hungary":   "#bc5090", "Romania":   "#ef5675", "Lithuania": "#ff764a",
            "Slovakia":  "#ffa600"
        };
        ...
        colorScale = function(name) {
            ...
            if (specificColors.hasOwnProperty(cleanName)) return specificColors[cleanName];
            return fallbackScale(cleanName);
        };

        // ==========================================
        // --- NUOVA LOGICA BOTTONI ANNO ---
        // ==========================================
        // Seleziona i bottoni dentro il container specifico del Sankey
        const yearButtons = d3.selectAll("#sankey-filter-container .btn-compact");

        yearButtons.on("click", function() {
            // 1. Gestione classe visiva active
            yearButtons.classed("active", false);
            d3.select(this).classed("active", true);

            // 2. Aggiornamento Anno
            const selectedYear = d3.select(this).attr("data-type");
        ...
        // --- LOGICA HELP (Richiede utils.js) ---
        if (typeof createChartHelp === "function") {
            createChartHelp("#sankey-help-container", "#sankey-chart-wrapper", {
                title: "How to read the Sankey Diagram",
                steps: [
                    "<strong>Flows:</strong> Lines represent flight routes from Origin (left) to Destination (right).",
                    "<strong>Thickness:</strong> Thicker lines mean more flights.",
                    "<strong>Interaction:</strong> Hover over links or nodes to see exact numbers.",
