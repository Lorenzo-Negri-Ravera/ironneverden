...
// --- 1. SETUP UI IMMEDIATO (Senza attendere i dati) ---
...
// --- CREAZIONE E ATTIVAZIONE LOADER ---
// Usiamo la classe CSS "chart-loader-overlay" definita nel foglio di stile
const loader = svgParent.append("div")
    .attr("class", "chart-loader-overlay"); // Appare subito perché nel CSS è display: flex
...
// Overlay bianco per il dettaglio
const overlay = svg.append("rect")
    .attr("width", width)
    .attr("height", height)
    .attr("fill", "#f8f9fa") // Stesso colore del background
    .style("display", "none")
    .style("opacity", 0);
...
Promise.all([
    ...
    function updateLegend(maxVal) {
        ...
        linearGradient.selectAll("stop").data(stops).enter().append("stop").attr("offset", d => (d * 100) + "%").attr("stop-color", d => colorScale(d * maxVal));
        legendGroup.append("text").attr("x", 0).attr("y", -8).style("font-family", "Fira Sans, sans-serif").style("font-size", "11px").style("font-weight", "600").style("fill", "#6c757d").text("Intensity (Number of Events)");
        legendGroup.append("rect").attr("width", legendWidth).attr("height", legendHeight).style("fill", "url(#linear-gradient)").attr("rx", 4);
        ...
        const axisGroup = legendGroup.append("g").attr("class", "legend-axis").attr("transform", `translate(0, ${legendHeight})`).call(axisBottom);
    ...
    function renderTooltipHtml(title, stats) {
        if (!stats) return `<div class='tooltip-header'><strong>${title}</strong></div>No events`;
        let html = `<div class='tooltip-header'><strong>${title}</strong></div>`;
        html += `<div><strong>Total Events: ${stats.total}</strong></div>`;
        if (stats.isEventSpecific) html += `<div style='font-size:11px; color:#888; margin-bottom:4px;'>Breakdown of ${stats.selectedType}:</div>`;
    ...
    function updateVisualization() {
        ...
        countries.on("mouseover", function (event, d) {
                ...
                const name = getGeoName(d);
                const stats = statsCache.get(name);
                d3.select(this).attr("fill", "orange").attr("stroke", "#000").attr("stroke-width", 1.5);
                tooltip.style("opacity", 1).html(renderTooltipHtml(name, stats)).style("left", (event.pageX + 15) + "px").style("top", (event.pageY - 15) + "px");
            })
            .on("mousemove", (event) => tooltip.style("left", (event.pageX + 15) + "px").style("top", (event.pageY - 15) + "px"))
            .on("mouseout", function (event, d) {
                const name = getGeoName(d);
                const stats = statsCache.get(name);
                d3.select(this).attr("stroke", "#fff").attr("stroke-width", 0.5).attr("fill", (stats && stats.total > 0) ? colorScale(stats.total) : "#eee");
                tooltip.style("opacity", 0);
            })
            .on("click", (event, d) => {
                const isoCode = getGeoISO(d);
                const name = getGeoName(d);
                if (isoCode) loadDetail(isoCode, name);
            });
    }
        ...
        function drawFeatures(selection, features, projection) {
            ...
            selection.selectAll("path").data(features).join("path")
                ...
                .on("mouseover", function (event, d) {
                    ...
                    tooltip.style("opacity", 1).html(renderTooltipHtml(regionName, stats)).style("left", (event.pageX + 15) + "px").style("top", (event.pageY - 15) + "px");
                })
                .on("mousemove", (event) => tooltip.style("left", (event.pageX + 15) + "px").style("top", (event.pageY - 15) + "px"))
                .on("mouseout", function (event, d) {
...
Promise.all([
    ...
    // --- Zoom Logic ---
    // MODIFICA: Aggiunto .translateExtent per bloccare il panning infinito
    /*
    const zoom = d3.zoom()
        .scaleExtent([1, 12])
        .translateExtent([[0, 0], [width, height]]) // <--- LIMITA LO SPOSTAMENTO AI BORDI DELL'SVG
        .on("zoom", (event) => {
            mapGroup.attr("transform", event.transform);
            // Applichiamo la trasformazione anche ai gruppi di dettaglio se sono visibili
    ...
    const zoom = d3.zoom()
        .scaleExtent([1, 12])
        // [[minX, minY], [maxX, maxY]]
        // Aumentando maxX (width + 1200) permettiamo di scorrere molto a destra verso la Russia
        // Aumentando maxY (height + 600) permettiamo di scorrere in basso
    ...
    // Nota: Ho aggiunto 'detailMapGroup' dentro l'evento zoom qui sopra per sicurezza, 
    // anche se solitamente viene gestito, ma è fondamentale che il limite si applichi a tutto.
    ...
    // --- Logica IBRIDA per la select del Country ---
    countrySelect.on("change", function () {
        const selectedCountry = this.value;
